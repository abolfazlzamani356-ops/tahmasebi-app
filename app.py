...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'tahmasebi-mega-erp-v12-final-secret-2026')

# رمز عبور مادر و نجات مدیریت برای مواقع فراموشی
MASTER_ADMIN_PASSWORD = 'king68abolfazl@68'

# تنظیم مسیر امن و استاندارد ذخیره‌سازی دیتابیس
instance_dir = os.path.join(app.root_path, 'instance')
os.makedirs(instance_dir, exist_ok=True)
db_file = os.path.join(instance_dir, 'tahmasebi_erp_v12.db')
...
<body class="bg-slate-100 min-h-screen text-slate-800 flex flex-col justify-between">
    ...
    <footer class="py-4 text-center text-xs text-gray-400 no-print border-t border-gray-200 mt-8">
        سامانه جامع فروشگاه‌های تخصصی طهماسبی • نسخه آنلاین ۱۲.۰
    </footer>
...
<!-- بخش آمار انبارداری با دکمه ویرایش و حذف در پنل مدیریت -->
<div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8 overflow-x-auto">
    <div class="flex justify-between items-center mb-4 border-b pb-3">
        <h3 class="text-md font-bold text-slate-800 flex items-center gap-2">
...
def download_backup():
    ...
    return send_file(db_file, as_attachment=True, download_name=f"Backup_Tahmasebi_{now_str}.db")
